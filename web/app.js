...
"NVIDIA": [
  ...
  disk: {
    ...
    "Отключение функций": [
      ...
      ["ReadyBoost OFF", "sc.exe config rdyboost start= disabled 2>$null", "safe", "Отключает ReadyBoost — не нужен для SSD.", null, "Не нужен для SSD. ReadyBoost — для HDD."],
...
const OPTIMIZE_PACK = {
  ...
  "NVIDIA": [
    ["NVIDIA Telemetry OFF", "sc.exe config NvTelemetryContainer start= disabled 2>$null | Out-Null; Stop-Service NvTelemetryContainer -Force -EA 0; reg add 'HKCU\\SOFTWARE\\NVIDIA Corporation\\NVControlPanel2\\Client' /v OptInOrOutPreference /t REG_DWORD /d 0 /f; Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object {$_.TaskName -like 'NvTm*' -or $_.TaskName -like 'NvProfileUpdater*'} | ForEach-Object { Disable-ScheduledTask -TaskName $_.TaskName -TaskPath $_.TaskPath -ErrorAction SilentlyContinue }; $LASTEXITCODE = 0", "Отключает службу телеметрии NVIDIA и фоновые задачи NvTm/NvProfileUpdater. Карта и драйвер продолжают работать.", "Применяется на ПК с видеокартой NVIDIA. Обновления Game Ready — вручную."],
  ],
  ...
  "Сбор данных (сводно)": [
    ...
    ["diagnosticshub OFF", "sc.exe config diagnosticshub.standardcollector.service start= disabled 2>$null | Out-Null; Stop-Service diagnosticshub.standardcollector.service -Force -EA 0; $LASTEXITCODE = 0", "Отключает Diagnostics Hub — службу сбора данных для отладки приложений.", "Не влияет на работу приложений."],
    ["Телеметрия OFF (службы)", "$k=@('DiagTrack','dmwappushservice','diagnosticshub.standardcollector.service','SgrmBroker','PcaSvc'); foreach($s in $k){ sc.exe config $s start= disabled 2>$null | Out-Null; Stop-Service $s -Force -EA 0 }; $LASTEXITCODE = 0", "Отключает 5 основных служб телеметрии Windows: DiagTrack, dmwappushservice, Diagnostics Hub, SgrmBroker и PcaSvc. Windows прекращает собирать диагностику и отправлять её Microsoft.", "Все 5 служб безопасны к отключению — для работы системы и игр ни одна не нужна."],
    ["Сбор данных OFF (безопасные)", "$k=@('WerSvc','wcncsvc','Wecsvc'); foreach($s in $k){ sc.exe config $s start= disabled 2>$null | Out-Null; Stop-Service $s -Force -EA 0 }; $LASTEXITCODE = 0", "Отключает безопасные сборщики данных: Windows Error Reporting (отчёты об ошибках), Windows Connect Now (данные Wi-Fi-подключений) и Windows Event Collector (журналы событий).", "Почта, обои, обновления и обычные функции Windows продолжают работать."],
  ],
...
// «Умная» оптимизация по выбору пользователя включает сводные твики
  ...
  // 5 служб телеметрии, безопасные сборщики данных, diagnosticshub) и
  // эко-режима сети. ДНС, оформление меню/виджетов и гибернацию она НЕ
  // трогает — это личные привычки, они остаются ручными твиками. Защиту ОС
  // автопакет не отключает. Сводные твики не вредят слабым системам,
  // поэтому даются всем профилям (эко-сеть — только десктопам).
...
const DATACOLL_TWEAKS = {
  "Телеметрия Microsoft": [
    ["Отключить DiagTrack (Main Telemetry)", 'sc.exe config DiagTrack start= disabled; Stop-Service DiagTrack -Force -EA 0; reg add "HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection" /v AllowTelemetry /t REG_DWORD /d 0 /f', "safe", "Главная служба телеметрии. Отключает сбор данных о использовании Windows."],
    ["Отключить dmwappushservice", 'sc.exe config dmwappushservice start= disabled; Stop-Service dmwappushservice -Force -EA 0', "safe", "WAP Push — служба push-уведомлений. Не нужна на ПК."],
    ["Отключить DiagnosticExecutionService", 'sc.exe config diagsvc start= disabled; Stop-Service diagsvc -Force -EA 0', "safe", "Diagnostic Execution Service — выполнение диагностических задач."],
    ["Отключить Diagnostic Service Host", 'sc.exe config WdiServiceHost start= disabled; Stop-Service WdiServiceHost -Force -EA 0', "safe", "Хост диагностических сервисов — поддержка телеметрии."],
    ["Отключить Diagnostic System Host", 'sc.exe config WdiSystemHost start= disabled; Stop-Service WdiSystemHost -Force -EA 0', "safe", "Системный хост диагностик — фоновый сбор данных."],
    ["Отключить diagnosticshub", 'sc.exe config diagnosticshub.standardcollector.service start= disabled; Stop-Service diagnosticshub.standardcollector.service -Force -EA 0', "safe", "Diagnostics Hub — сбор данных для отладки приложений."],
    ["Отключить Windows Error Reporting", 'sc.exe config WerSvc start= disabled; Stop-Service WerSvc -Force -EA 0', "safe", "Отправка отчётов об ошибках Microsoft. Можно отключить без последствий."],
  ...
  "Следящие службы": [
    ["Отключить RetailDemo", 'sc.exe config RetailDemo start= disabled; Stop-Service RetailDemo -Force -EA 0', "safe", "Демо-режим магазина. Полностью безопасно отключить."],
    ["Отключить MapsBroker (Карты)", 'sc.exe config MapsBroker start= disabled; Stop-Service MapsBroker -Force -EA 0', "safe", "Скачанные карты. Не нужны — отключаем."],
    ["Отключить lfsvc (Геолокация)", 'sc.exe config lfsvc start= disabled; Stop-Service lfsvc -Force -EA 0', "safe", "Служба геолокации. Отключает отслеживание местоположения."],
    ["Отключить PcaSvc (Совместимость)", 'sc.exe config PcaSvc start= disabled; Stop-Service PcaSvc -Force -EA 0', "safe", "Program Compatibility Assistant — фоновый мониторинг запуска."],
    ["Отключить wisvc (Windows Insider)", 'sc.exe config wisvc start= disabled; Stop-Service wisvc -Force -EA 0', "safe", "Windows Insider Service. Отключает сбор данных инсайдеров."],
    ["Отключить DsSvc (Sharing)", 'sc.exe config DsSvc start= disabled; Stop-Service DsSvc -Force -EA 0', "safe", "Data Sharing Service — обмен данными между приложениями."],
    ["Отключить DsmSvc (Device Setup)", 'sc.exe config DsmSvc start= disabled; Stop-Service DsmSvc -Force -EA 0', "safe", "Device Setup Manager — автоматическая настройка устройств."],
    ["Отключить PhoneSvc", 'sc.exe config PhoneSvc start= disabled; Stop-Service PhoneSvc -Force -EA 0', "safe", "Phone Service — поддержка телефонных функций. Не нужна на ПК."],
    ["Отключить icssvc (Hotspot)", 'sc.exe config icssvc start= disabled; Stop-Service icssvc -Force -EA 0', "safe", "Windows Mobile Hotspot — раздача интернета. Можно отключить."],
  ],
  "Реклама и трекинг": [
  ...
  "NVIDIA и приложения": [
    ["NVIDIA Telemetry OFF", 'sc.exe config NvTelemetryContainer start= disabled; Stop-Service NvTelemetryContainer -Force -EA 0; reg add "HKCU\\SOFTWARE\\NVIDIA Corporation\\NVControlPanel2\\Client" /v OptInOrOutPreference /t REG_DWORD /d 0 /f; Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object {$_.TaskName -like "NvTm*" -or $_.TaskName -like "NvProfileUpdater*"} | ForEach-Object { Disable-ScheduledTask -TaskName $_.TaskName -TaskPath $_.TaskPath -ErrorAction SilentlyContinue }', "safe", "Служба и задачи телеметрии NVIDIA (NvTelemetryContainer, NvTmMon/NvTmRep/NvProfileUpdater) отключены.", null, "Карта и драйвер продолжают работать. Обновления Game Ready — вручную."],
  ...
  "Службы (Xbox/Bluetooth/печать)": [
    ["Xbox Services OFF", 'sc.exe config XblAuthManager start= disabled; sc.exe config XblGameSave start= disabled; sc.exe config XboxNetApiSvc start= disabled; sc.exe config XboxGipSvc start= disabled; Stop-Service XblAuthManager,XblGameSave,XboxNetApiSvc,XboxGipSvc -Force -EA 0', "safe", "Все службы Xbox (авторизация, Game Save, API, GIP) отключены.", null, "Xbox Game Pass / облачные функции перестанут работать."],
    ["Bluetooth OFF", 'sc.exe config bthserv start= disabled; sc.exe config BTAGService start= disabled; Stop-Service bthserv,BTAGService -Force -EA 0', "warn", "Полностью отключает Bluetooth на уровне служб.", null, "Блютуз-мышь/наушники перестанут работать."],
    ["Print Spooler OFF", 'sc.exe config Spooler start= disabled; sc.exe config PrintNotify start= disabled; Stop-Service Spooler,PrintNotify -Force -EA 0', "warn", "Отключает службы печати (Spooler, PrintNotify).", null, "Печать и виртуальные принтеры (PDF) перестанут работать."],
    ["Remote Registry OFF", 'sc.exe config RemoteRegistry start= disabled; Stop-Service RemoteRegistry -Force -EA 0', "safe", "Удалённый доступ к реестру отключён — стандарт безопасности.", null, "Удалённое управление реестром не используется на домашних ПК."],
    ["Terminal Services OFF", 'sc.exe config TermService start= disabled; Stop-Service TermService -Force -EA 0', "warn", "Отключает службу удалённого рабочего стола.", null, "RDP / Удалённый рабочий стол перестанут работать."],
  ],
